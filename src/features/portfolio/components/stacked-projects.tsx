...
export const StackedProjects = () => {
  ...
  return (
    <section id="work" className="relative z-20 bg-[#FFFFFF] border-t">
      {/* Section Header */}
      <div ref={headingRef} className="container mx-auto px-6 md:px-12 pt-[var(--space)] pb-[var(--space-xl)]">
        ...
        {projects.map((project, index) => {
          ...
          return (
            <div 
              ...
              style={{ 
                zIndex: index + 1,
                top: headingVisible ? `${stickyTop}px` : '100vh',
                transition: 'top 0.6s cubic-bezier(0.4,0,0.2,1)',
              }}
            >
              <div className={cn(
                "relative w-full max-w-[1100px] h-full card-surface flex flex-col justify-start p-8 md:p-10",
                project.color
              )}>
                {/* Content Layout */}
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-14 items-start flex-grow overflow-hidden">
                  ...
                  <div className="lg:w-1/2 relative h-[250px] lg:h-full w-full group">
                    <div className="relative w-full h-full card-surface transition-transform duration-700 group-hover:scale-[1.01] bg-slate-50">
                      {image && (
                        <Image 
                          src={image.imageUrl} 
